...
class ComprobanteDomicilio extends Component {
    ...
    pickImage = async () => {
        if(this.state.isConnected != false){
            ...
            if(this.state.image != ""){
                this.props.navigation.navigate('CargarComprobante',{image: this.state.image, id_usuario: this.state.id_usuario, id_cat_documento:9, estado: this.state.estado})
            }
        }
    };
    ...
    openCamera = () => {
        if(this.state.isConnected != false){
            this.props.navigation.navigate('Camara',{image: this.state.image, id_usuario: this.state.id_usuario, id_cat_documento:9, estado: this.state.estado});
        }
    };
