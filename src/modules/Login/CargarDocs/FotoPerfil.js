...
class FotoPerfil extends Component {
    ...
    pickImage = async () => {
        if(this.state.isConnected != false){
            ...
            if(this.state.image != ""){
                this.props.navigation.navigate('CargarFoto',{image: this.state.image, id_usuario: this.state.id_usuario, id_cat_documento:3, estado: this.state.estado})
            }
        }
    };
    ...
    openCamera = () => {
        if(this.state.isConnected != false){
            this.props.navigation.navigate('Camara',{image: this.state.image, id_usuario: this.state.id_usuario, id_cat_documento:3, estado: this.state.estado})
        }
    };
                    ...
                    <Row>
                        {this.state.estado == 0
                            ?<Col style={{display:'flex', alignItems:'center',alignContent:'center', marginTop:20}}>
                                <Thumbnail style={{ height: 200, width: 200, resizeMode: 'contain', borderRadius:100}} square source= {{uri:this.state.imageCargada}} />
                             </Col>
